...
+ (void)displayHudError:(NSString *)errStr
{
...
+ (void)displayTipInfo:(NSString *)infoStr withTime:(double)time onView:(UIView *)sView
{
...
+ (void)displayHudErrorWithYHeighten:(NSString *)errStr
{
    if (![errStr isKindOfClass:[NSString class]]
        || errStr.length == 0) {
        return;
    ...
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:[UIApplication sharedApplication].keyWindow];
    hud.frame = CGRectMake(hud.frame.origin.x, 170, hud.frame.size.width, hud.frame.size.height);
    hud.labelText = errStr;
    hud.labelFont = [UIFont systemFontOfSize:14.0f];
    hud.removeFromSuperViewOnHide = YES;
    [hud setMode:MBProgressHUDModeCustomView];
...
+ (void)displayHudErrorWithVercial:(NSString *)errStr
{
    if (![errStr isKindOfClass:[NSString class]]
        || errStr.length == 0) {
        return;
    ...
    [hud setTransform:CGAffineTransformMakeRotation(radians)];
    [hud show:YES];
    double fDelay = 1.0f;
    if (errStr.length > 10) {
        fDelay = 2.0f;
...
+ (MBProgressHUD *)showHUDText:(NSString *)desStr animated:(BOOL)animated {
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:[UIApplication sharedApplication].keyWindow];
    hud.center = CGPointMake([UIApplication sharedApplication].keyWindow.center.x, [UIApplication sharedApplication].keyWindow.center.y);
    hud.labelText = desStr;
    hud.labelFont = [UIFont systemFontOfSize:14.0f];
    //	[view addSubview:hud];
    [[UIApplication sharedApplication].keyWindow addSubview:hud];
	[hud show:animated];
...
-(BOOL)hideOnMainThread:(NSNumber *)animated
{
	if ([NSThread isMainThread]) {
		[self hide:[animated boolValue]];
	} else {
        [self performSelectorOnMainThread:@selector(hide:) withObject:animated waitUntilDone:NO];
    }
    return YES;
}
	...
	for (UIView *v in [[UIApplication sharedApplication].keyWindow subviews]) {
		if ([v isKindOfClass:[MBProgressHUD class]]) {
            ...
            if (viewToRemove != nil) {
                ...
                HUD.removeFromSuperViewOnHide = YES;
                [HUD hide:animated];
                //                [HUD hideOnMainThread:[NSNumber numberWithBool:animated]];
                viewToRemove = nil;
                //                return YES;
            } else {
                return NO;
...
+ (MBProgressHUD *)showHUDText:(NSString *)desStr forView:(UIView *)view animated:(BOOL)animated
{
...
+ (void)displayHudError:(NSString *)errStr forView:(UIView *)view
{
    [self displayHudError:errStr forView:view fontSize:14.0f];
}

+ (void)displayHudError:(NSString *)errStr forView:(UIView *)view fontSize:(CGFloat)fontSize {
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    hud.labelText = errStr;
...
+ (BOOL)hideHUDForCustomView:(UIView *)view animated:(BOOL)animated
{
	UIView *viewToRemove = nil;
...
- (void)layoutSubviews {
	...
	CGSize detailsLabelSize = MB_MULTILINE_TEXTSIZE(_detailsLabel.text, _detailsLabel.font, maxSize, _detailsLabel.lineBreakMode);
	...
	_detailsLabel.frame = detailsLabelF;
